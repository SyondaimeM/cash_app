...
class ReportsController extends Controller
{
    ...
    public function bitcoin(Request $request)
    {
        ...
        if (!empty($query_data)) {
            ...
            $to = $query_data['to'];
            if ($bank_id == '') {
                $data_buy = $data_buy->whereBetween('date', [$from, $to]);
                $data_sale = $data_sale->whereBetween('date', [$from, $to]);
                $data_withdrawal = $data_withdrawal->whereBetween('date', [$from, $to]);
            } else {
    ...
    public function outtobank(Request $request)
    {
        $banks = Bank::all();
        $data = Engagement::select('*')->where('transaction_type', 'Cash out');
        ...
        if (!empty($query_data)) {
            ...
            $to = $query_data['to'];
            if ($bank_id != '') {
                $data = $data->Bank($bank_id)->whereBetween('date', [$from, $to]);
            } else {
        ...
        $data = $data->get();
        // dd($data);
        return view('admin.record.outtobank', compact('data', 'query_data', 'banks'));
    }
